...
async function runSmokeTest() {
  ...
  try {
    ...
    const response = await fetch(`${API_URL}/api/jobs`, {
      ...
      body: JSON.stringify({
        goal: "python developers with activity in the last 90 days", // Exact same goal as manual command
        dryRun: false, // Enable real execution to see tqdm progress
        config_path: "cmo_agent/config/smoke_prompt.yaml", // Use exact same config as manual command
        metadata: {
          threadId,
          autopilot_level: 0,
          budget_per_day: 1,
